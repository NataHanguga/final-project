...
export class TeachersComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.teacherItem = this.formBuilder.group({
      ...
      category: ['first category teacher', Validators.required],
    ...
    this.getPosition();
    this.getPedTitle();
    // this.categoryList.map((index, item) => {
    // if (this.teacherItem.value.category === item[index].position) {
    //   this.teacherItem.value.grade = item[index].level;
    // }});
  }
  ...
  public distinct = (value, index, self) => {
  ...
  public getPosition() {
    this.gradeService.getGrades().subscribe(
      res => {
        ...
        console.log(1111, this.gradeList, this.categoryList);
      });
  }
