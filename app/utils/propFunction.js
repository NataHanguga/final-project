...
const tableData = (body) => {
    ...
    for (let i = 0; i < propItem.length; i++) {
        if (propItem[i].level === '13') {
            propItem[i].serviceYears = +(propItem[i].price * 0.3).toFixed(2)
            propItem[i].twentyPercents = +(propItem[i].serviceYears * 0.3).toFixed(2)
        } else {
            propItem[i].serviceYears = 0
            propItem[i].twentyPercents = 0
        }

        if (propItem[i].position === 'Прибиральник службових приміщень') {
        ...
        propItem[i].priceFondByYear = (propItem[i].priceFondByMounth * 12).toFixed(2);
...
exports.tableData1 = (body) => {
    if (body.staffUnitsAmounts !== 0) {
        ...
        if (body.level === '13') {
            body.serviceYears = +(body.price * 0.3).toFixed(2)
            body.twentyPercents = +(body.serviceYears * 0.3).toFixed(2)
        } else {
            body.serviceYears = 0
            body.twentyPercents = 0
        }

        if (body.position === 'Прибиральник службових приміщень') {
