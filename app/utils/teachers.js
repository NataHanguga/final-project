...
const price = (body) => {
    for (let i = 0; i < gradeData.length; i++) {
        if (Number(body) === gradeData[i].level) {
...
exports.paramsOfTeacher = (body) => {
    body.price = price(body.grade); // ставка
...
exports.summ = (data) => {
    ...
    summArray.push(
        ...
        +valueChange.summ(data, 'inAll').toFixed(2), //7
       /* +valueChange.summ(data, 'tarifAndIncrease').toFixed(2), //8
        +(summArray[7] / summArray[3]).toFixed(2), //9
        +(summArray[7] + summArray[8]).toFixed(2)*/ ) //10
    return summArray
}